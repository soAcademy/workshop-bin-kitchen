...
const Navbar = () => {
  ...
  return (
    <div className={`flex flex-col ${isNavOpen ? "" : ""} `}>
      ...
      <div className={`bg-black opacity-10 fixed h-screen w-screen md:hidden duration-500 ${isNavOpen ? "z-20" : "opacity-0 z-10"}`} onClick={()=>setIsNavOpen(false)}></div>
    </div>
  );
};
