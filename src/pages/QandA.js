...
const QandA = () => {
  ...
  //  todo 1 : สร้าง State toggle เซ็ตค่าเริ่มต้นเป็น Array ที่มีจำนวนสมาชิกเท่ากันกับ Array Faq และ .map() ออกมาให้มี Output เป็น false มั้งหมด
  const [toggles, setToggles] = useState(
    [...Array(faqs.length)].map(() => false) // outputs ==> [false,false,false,false,false]
  );
  // todo 2 : เมื่อมีการคลิกที่คำถาม index[n] ให้ทำการเปลี่ยนแปลงค่า ใน Array toggles ใน useState
  const updateFaqToggle = (index) => {
    // console.log("toggles", toggles); // outputs ==> [false,false,false,false,false]
    // todo 2.1 : เมื่อคลิกคำถามแล้วให้ สร้าง temporary toggles โดยการ Clone array มาจากตัวแปร toggles ที่อยู่ใน useState
    // สำหรับใช้ในการเปลี่ยนแปลงค่าโดยไม่ให้เกิดการเปลี่ยนแปลงโดยตรงกับตัวแปรใน useState พอเปลี่ยนแปลงเสร็จแล้วค่อยยัดกลับคืนไปในเสตททีหลัง
    const newToggles = [...toggles];
    // console.log("index", index); // outputs ==> index No. 0-4
    // todo 2.2 : เมื่อคลิกคำถามแล้วให้ ให้เอาค่า index ของคำถามนั้น ไปเปลี่ยนแปลงใน newToggles ที่ index เดียวกัน ให้เป็นค่าตรงกันข้าม
